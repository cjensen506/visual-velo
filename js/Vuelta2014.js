            ...
            d3.csv("Data/Vuelta2014Results.csv", 
                ...
                d3.csv("Data/Vuelta2014_Stage_Cartoon.csv", function(error, RawStages) {
                    ...
                    xScale.domain([1, 21.5]);

                    yScale.domain([1, maxRiders]); //Rider Result Y scale
                    yScaleConstant.domain(yScale.domain());
                    
                    //check if current max date is less that gc time  (zoom out for stage 2)
                    if (maxDate < GCTime){
                        TScaleMax = GCTime
                    } else {
                        TScaleMax = maxDate
                    ...
                    yTimeScale.domain([d3.time.format("%S").parse("0"), TScaleMax]); //Rider Time Scale
                    yTimeScaleConstant.domain(yTimeScale.domain());

                    Py.domain([0, 1]); //Stage Profile Y scale
             ...
             //       dropDownCountry.append("option").attr("value","");
                ...
                //    $("#RiderDrop").chosen({max_selected_options: 2 });
                    ...
                    var stages = ProfileSvg.append("svg:g")
                        ...
                        .attr("d", function(d) {
                            ans="";
                            if (d.Type == "Individual time trial"){
                                ans="";
                            }
                            else if (d.Type == "Flat stage"){
                                ans="";
                            }
                            else if (d.Type == "Hilly stage"){
                                ans = "M " + xScale(+d.Stage-.5) + " " + Py(0) + " L " + xScale(+d.Stage-.25) +" "+Py(.25) + " L " + xScale(+d.Stage) +" "+Py(0)+ " L " + xScale(+d.Stage+.25) +" "+Py(.25)+ " L " + xScale(+d.Stage+.5) +" "+Py(0);
                            }
                            else if (d.Type == "Medium-mountain stage"){
                                ans = "M " + xScale(+d.Stage-.5) + " " + Py(0) + " L " + xScale(+d.Stage) +" "+Py(.5) + " L " + xScale(+d.Stage+.5) +" "+Py(0);
                            }
                            else if (d.Type == "Mountain stage"){
                                ans = "M " + xScale(+d.Stage-.5) + " " + Py(0) + " L " + xScale(+d.Stage) +" "+Py(1) + " L " + xScale(+d.Stage+.5) +" "+Py(0);
                            }
                            return ans;
                        })
...
/*
                    ...
                    */ 
                    ...
                    ProfileSvg.append("text")
                        ...
                        .style("font-size", "16px")
                        .style("text-anchor","middle")
                        .attr("transform","translate(9,0) rotate(-90," + xScale(1)+",50)")
                        .attr("y",50)
                        .attr("x",xScale(1))
                        .text("TTT");
                    ...
                    ProfileSvg.append("text")
                        ...
                        .style("font-size", "16px")
                        .style("text-anchor","middle")
                        .attr("transform","translate(9,0) rotate(-90," + xScale(10)+",50)")
                        .attr("y",50)
                        .attr("x",xScale(10))
                        .text("ITT");
                    ...
                    ProfileSvg.append("text")
                        ...
                        .style("font-size", "16px")
                        .style("text-anchor","middle")
                        .attr("transform","translate(9,0) rotate(-90," + xScale(21)+",50)")
                        .attr("y",50)
                        .attr("x",xScale(21))
                        .text("ITT");
...
*/
                  ...
                  //          var xPosition = xScale(StageToDistance(currentStage)) + 0 + svgSlider.node().getBoundingClientRect().width - window.pageXOffset;
                            var xPosition = xScale(currentStage+.5) + $("#RiderGraph").position().left +20;
                        ...
                        .each(function(d){ //if they have abandoned add mark
                            if(d.values.length < currentStage){
                                d3.select(this.parentNode)
                                    .append("circle")
                                    .attr("class","AbandonMark")
                                    .attr("r", 2)
                                    .attr("cx",xScale(d.values.length+.5))
            ...
            function PathString(DataTest) {
                if (currentDisplay == 0) { //Rank scale
                 //   var Pstring = "M " + xScale(.5) + " " + yScale(1);
                    var Pstring = "M " + xScale(1.5) + " " + yScale(DataTest[0]);
                    for (var i = 2; i <= currentStage; i++) {
                        if (!(isNaN(+DataTest[i-1]))) {
                            Pstring = Pstring + " L " + xScale(i+.5) + " " + yScale(DataTest[i-1]);
                        } else {
                            break
                        }
                    }
                  ...
                  //  var Pstring = "M " + xScale(.5) + " " + yTimeScale(d3.time.format("%M").parse("0"));
                    var Pstring = "M " + xScale(1.5) + " " + yTimeScale(DataTest[0]);
                    for (var i = 2; i <= currentStage; i++) {
                        if (!(isNaN(+DataTest[i-1]))) {
                            Pstring = Pstring + " L " + xScale(i+.5) + " " + yTimeScale(DataTest[i-1]);
                        } else {
                            break
                        }
                    }
